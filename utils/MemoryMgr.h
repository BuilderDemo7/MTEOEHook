...
namespace Memory
{
	...
	{
		*(uint8_t*)address = ( nType == PATCH_JUMP || nType == PATCH_JUMPHACK) ? 0xE9 : 0xE8;
		if (nType == PATCH_JUMPHACK)
		{
			WriteOffsetValue( (intptr_t)address + 1, ( (int*)((*(int*)( (int)(hook)+1 ))) ) + DynBaseAddress(0) );
			return;
		}
		InjectHook(address, hook);
		//WriteOffsetValue((intptr_t)address + 6, 0x90); // NOP
	}
